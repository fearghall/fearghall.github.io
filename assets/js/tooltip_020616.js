...
$(document).ready(function () {
	$('#resetAfterSubmission').toggle();
});

$(document).ready(function () {
	$('#resetAfterSubmission').on('click', function() {
		$('#resetAfterSubmission').toggle();
		$('#submit').toggle();
		$('#reset').toggle();
	});
...
$(document).ready(function () {
	jQuery.validator.addMethod("greaterThan", 
		...
		var date2Compare = parseDMY($('input[name="' + params + '"]').val());

		if (!/Invalid|NaN/.test(new Date(value))) {
			return new Date(value).setHours(0,0,0,0) > new Date(date2Compare).setHours(0,0,0,0);
		}
	...
	jQuery.validator.addMethod("notInPast", 
        ...
        value=parseDMY(value);
		 if (!/Invalid|NaN/.test(new Date(value))) {
			return ((new Date(value).setHours(0,0,0,0) == new Date().setHours(0,0,0,0)) || (new Date(value).setHours(0,0,0,0) >   new Date().setHours(0,0,0,0)));
		}

		return isNaN(value) && isNaN($(params).val()) 
				|| (Number(value) >= Number($(params).val())); 
	},'Must be greater than or equals today.');	
    ...
    $('#bookingForm input[type="text"]').tooltipster({
        trigger: 'custom',
        onlyOne: false,
        position: 'right'
    });
	
	 $('#bookingForm input[type="email"]').tooltipster({
        trigger: 'custom',
        onlyOne: false,
        position: 'right'
    });

	 $('#bookingForm input[type="date"]').tooltipster({
        trigger: 'custom',
        onlyOne: false,
        position: 'right'
    });
	
	$('#bookingForm select').tooltipster({
        trigger: 'custom',
        onlyOne: false,
        position: 'right'
    });	
	...
	function formToJSON() {
        ...
        "longname": $('input[name="longname"]').val(),
		 ...
		 "numrooms": $('select[name="numrooms"]').val(),
		 "numpeeps": $('select[name="numpeeps"]').val()
        });
	}
	
	function disableSubmit() {
		//Adds text
		//$('#submit').after("Your was processed");
		$('#submit').toggle();	
		$('#reset').toggle();
	};
	
	function enableSubmitAfterSucesss() {
		//Adds text
		$("#resetAfterSubmission").prop('value', 'Your was booking was processed');
		$('#resetAfterSubmission').toggle();
	};
	
	function enableSubmitAfterFailure() {
		//Adds text
		$("#resetAfterSubmission").prop('value', 'There was a problem with the booking please call Mary on 091 595188');
		$('#resetAfterSubmission').toggle();
	};
    ...
    $('#bookingForm').validate({
        ...
        messages: {
			...
			longname: "Please enter your first name",
			phone: "Please enter your phone number",
            numrooms: "Please enter the number of rooms you wish to book",
			checkin: {
                required: "Please enter a valid date",
                greaterThan: "Your password must be at least 5 characters long",
				notInPast: "The date should not be in the past"
            },
			checkout: {
                required: "Please enter a valid date",
                greaterThan: "Checkout needs to be after check in date",
				notInPast: "The date should not be in the past"
            },
			numpeeps: "Please enter the number of people that you wish to book for"
        },
		
		

		 submitHandler: function (form) { 
			//Work for non json cross browsers
			/*
			disableSubmit()
			var formData = $(form).serialize(); 
			jQuery.support.cors = true;
			request = $.ajax({
				url : "http://localhost:8080/RESTfulJK/users/post",
				type: "POST",
				data:formData,
				success: function() { 
						var alertMsg = "The booking request has been processed, Mary will contact you with a confirmation as soon as possible. ";
						var dateCheckin = $('input[name="checkin"]', form).val();
						if(new Date(dateCheckin).setHours(0,0,0,0) == new Date().setHours(0,0,0,0)) {
							alertMsg += "Although you want to stay tonight, so the best option would be to call on +353 (0)91 595188. ";
						} else {
							alertMsg += "Any queries just call on  +353 (0)91 595188. ";
						}
						alertMsg += "Thanks ";
						alert(alertMsg); 
						form.reset();
						},
				error: function() { alert('There was a problem with your submission, please call Mary on +353 (0)91 595188'); },
				
			});
			*/
			
			// Manages to call WS fails on NotSupportedException: Cannot consume content type/
			/*	request = $.ajax({
				url : "http://localhost:8080/RESTfulJK/users/post2",
				type: "POST",
				headers: {
					'Access-Control-Allow-Origin':'http://localhost:8080/RESTfulJK/users/post2',
					'Content-Type':'application/json'
				},
				contentType: 'application/json',
				dataType: "json",
				//data: {longname: 'id'},
				data: '{\"longname\":\"FOL Hardcoded\",\"phone\":null,\"comment\":null,\"checkin\":null,\"checkout\":null,\"email\":null,\"numrooms\":0,\"numpeeps\":0,\"mailSent\":false}',
				success: function() { 
						var alertMsg = "The booking request has been processed, Mary will contact you with a confirmation as soon as possible. ";
			...
			*/
			...
			//This works as a basic json submission
			/**/
			disableSubmit();
			var formData = $(form).serialize(); 
			jQuery.support.cors = true;
			request = $.ajax({
				...
				url : "https://webservice.carraroehouse.com:8443/RESTfulJK/users/post/",
				type: "POST",
				data:formToJSON(),
				contentType: 'application/json',
				success: function() { 
						...
						alert(alertMsg); 
						//form.reset();
						enableSubmitAfterSucesss();
						},
				error: function() { 
						alert('There was a problem with your submission, please call Mary on +353 (0)91 595188'); 
						enableSubmitAfterFailure();
						},
